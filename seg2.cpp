#include <stdio.h>
#include <stdlib.h>
//#define struct Seg struct Seg
#define mid (l+r>>1)
char*sp,str[1000000],*p=str;
int n,m,a[20001][2],s,ra[20001],K,Y=1,I,b[20000],v[10001]={-2,-2};
struct Seg{struct Seg*c[2];int s}*R[10001],st[2500000],*stp=st,*T[10001];
getint(){
	int re=0;
	while(*p<'0'||*p>'9')p++;
...
struct Seg*Build(l,r){
	struct Seg*node=stp++;
	if(l+1!=r)node->c[0]=Build(l,mid),node->c[1]=Build(mid,r);
	return node;
}
struct Seg*SegInsert(x,l,r)struct Seg*x;int l,r;{
	struct Seg*node=stp++;node->s=x->s+Y;
	if(l+1!=r){
