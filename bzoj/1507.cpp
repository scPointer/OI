...
void print(int pos,int tail,int p)
{
	int lcnt=cnt[son[p][0]];
	if(p==0) return;
	pushdown(p);
	if(pos<=lcnt) print(pos,Min(tail,lcnt),son[p][0]);
	if(pos<=lcnt+1 && lcnt+1<=tail) putchar(val[p]);
	if(lcnt+1<tail) print(Max(pos-lcnt-1,1),tail-lcnt-1,son[p][1]);
}
void find(int pos,int slen)
{
	print(pos,pos+slen-1,root);
//	return val[p];
}
...
int main()
{
	...
	for(int Q=RD();Q;Q--)
	{
		scanf("%s",rds);//while(rds[0]<32 || rds[0]>126) scanf("%s",rds);
		odr=rds[0];
		if(odr=='M')  ptr=RD();
		else if(odr=='P') ptr--;
		else if(odr=='N') ptr++;
