...
inline Matrix getFib(int p)
{
	if(p<100000) return ks[p];
...
void cdcdqqq(int l,int r)
{
	...
	for(int i=1;i<=cnt;i++)
	{
		...
		{
			ans[tag[i].id]+=mtxnow.v[0][0];
			if(ans[tag[i].id]>=mo) ans[tag[i].id]-=mo;
			ans[tag[i].id]+=mtxnow.v[0][1];
			if(ans[tag[i].id]>=mo) ans[tag[i].id]-=mo;
		}
	}
//	printf("solve %d %d\n",l,r);
